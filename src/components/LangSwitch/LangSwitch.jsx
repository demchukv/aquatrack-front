...
const LangSwitch = () => {
  return (
    <div className={css.langSwitch}>
      <select name="lang" onChange={e => chLang(e.target.value)} className={css.langSelect} defaultValue={i18next.language}>
        <option value={LOCALES.UA.code} disabled={i18next.language === LOCALES.UA.code}>{LOCALES.UA.name}</option>
        <option value={LOCALES.EN.code} disabled={i18next.language === LOCALES.EN.code}>{LOCALES.EN.name}</option>
      </select>
